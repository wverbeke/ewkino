...
void treeReader::Analyze(){
    ...
    for(size_t sam = 0; sam < samples.size(); ++sam){
        ...
        for(long unsigned it = 0; it < nEntries; ++it){
            ...
            if( !tzq::isWZControlRegion(controlRegion) || (_metJECDown > 50 &&  bdtVariableMap["numberOfbJets"] == 0 ) ){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapDown["JEC_2016"][controlRegion][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            ...
            if( !tzq::isWZControlRegion(controlRegion) || (_metJECUp > 50 &&  bdtVariableMap["numberOfbJets"] == 0 ) ){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapUp["JEC_2016"][controlRegion][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            }
    ...
    for(unsigned cr = 0; cr < nCr; ++cr){
       for(unsigned dist = 0; dist < nDist; ++dist){
           plotDataVSMC(mergedHists[cr][dist][0], &mergedHists[cr][dist][1], &proc[0], mergedHists[cr][dist].size() - 1, "plots/tZq/2016/controlR/" + crNames[cr] + "/" + histInfo[dist].name() + "_" + crNames[cr] + "_2016", "tzq", false, false, "35.9 fb^{-1} (13 TeV)", totalSystUnc[cr][dist], isSMSignal);             //linear plots

           plotDataVSMC(mergedHists[cr][dist][0], &mergedHists[cr][dist][1], &proc[0], mergedHists[cr][dist].size() - 1, "plots/tZq/2016/controlR/" + crNames[cr] + "/" + histInfo[dist].name() + "_"  + crNames[cr] + "_2016" + "_log", "tzq", true, false, "35.9 fb^{-1} (13 TeV)", totalSystUnc[cr][dist], isSMSignal);    //log plots
       }
    ...
    for(unsigned cr = 0; cr < nCr; ++cr){
        ...
        for(unsigned p = 0; p < nBkg + 1; ++p){
            for(unsigned bin = 0; bin < nBinsFit; ++bin){
               systNames[nGeneralSystematics + p*nBinsFit + bin]  = processNames[p] + "_stat_" + crNames[cr] + "_2016_bin_" + std::to_string(bin + 1);
            }
        }

        //set BDT shape histogram
        TFile* shapeFile =TFile::Open((const TString&) "./datacards/shapes/shapeFile_"  + crNames[cr] +  "_2016.root", "recreate");
        ...
        analysisTools::printDataCard( mergedHists[cr][0][0]->GetSumOfWeights(), mergedHists[cr][0][1]->GetSumOfWeights(), "tZq", bkgYields, proc.size() - 2, bkgNames, systUnc, nSyst, &systNames[0], systDist, "datacards/datacard_" + crNames[cr] + "_2016", true, "shapes/shapeFile_"  + crNames[cr] + "_2016");
