...
void treeReader::Analyze(){
    ...
    for(unsigned cr = 0; cr < nCr; ++cr){
        for(unsigned dist = 0; dist < nDist; ++dist){
            plotDataVSMC(mergedHists[cr][dist][0], &mergedHists[cr][dist][1], &proc[0], mergedHists[cr][dist].size() - 1, "plots/tZq/2017/controlR/" + crNames[cr] + "/" + histInfo[dist].name() + "_" + crNames[cr] + "_2017", "tzq", false, false, "41.5 fb^{-1} (13 TeV)", nullptr, isSMSignal);             //linear plots

            plotDataVSMC(mergedHists[cr][dist][0], &mergedHists[cr][dist][1], &proc[0], mergedHists[cr][dist].size() - 1, "plots/tZq/2017/controlR/" + crNames[cr] + "/" + histInfo[dist].name() + "_"  + crNames[cr] + "_2017" + "_log", "tzq", true, false, "41.5 fb^{-1} (13 TeV)", nullptr, isSMSignal);    //log plots
        }
    }
    /*    
