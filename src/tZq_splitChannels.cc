...
void treeReader::Analyze(){
    ...
    const unsigned nFlav = 4;               //number of flavor channels 
    const std::string mllNames[nMll] = {"onZ"};
    const std::string catNames[nCat] = {"1bJet23Jets", "1bJet4Jets", "2bJets"};
    const std::string flavNames[nFlav] = {"eee", "eem", "emm", "mmm"};

    //initialize vector holding all histograms
    std::vector< std::vector < std::vector< std::vector< std::vector< std::shared_ptr< TH1D > > > > > > hists(nMll);
    for(unsigned m = 0; m < nMll; ++m){
        hists[m] = std::vector< std::vector < std::vector< std::vector < std::shared_ptr< TH1D > > > > >(nCat);
        for(unsigned cat = 0; cat < nCat; ++cat){
            hists[m][cat] = std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > >(nFlav);
            for(unsigned flav = 0; flav < nFlav; ++flav){
                hists[m][cat][flav] = std::vector < std::vector< std::shared_ptr< TH1D > > >(nDist); 
                for(unsigned dist = 0; dist < nDist; ++dist){
                    for(size_t sam = 0; sam < samples.size() + 1; ++sam){
                        if(sam < samples.size()){
                            hists[m][cat][flav][dist].push_back(histInfo[dist].makeHist( flavNames[flav] + catNames[cat] + mllNames[m] + samples[sam].getUniqueName()) );
                        } else {

                            //extra histogram for nonprompt prediction
                            hists[m][cat][flav][dist].push_back( histInfo[dist].makeHist( flavNames[flav] + catNames[cat] + mllNames[m] + "nonprompt") ); 
                        }
                    }
                }
            }
    ...
    std::map < std::string, std::vector< std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > > >  > uncHistMapUp;
    for( auto& key : uncNames ){
        uncHistMapDown[key] = std::vector < std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > > >(nMll);
        uncHistMapUp[key] = std::vector < std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > > >(nMll);
        for(unsigned m = 0; m < nMll; ++m){
            uncHistMapDown[key][m] = std::vector< std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > > >(nCat);
            uncHistMapUp[key][m] = std::vector< std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > > >(nCat);
            for(unsigned cat = 0; cat < nCat; ++cat){
                uncHistMapUp[key][m][cat] = std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > >( nFlav );
                uncHistMapDown[key][m][cat] = std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > >( nFlav );
                for(unsigned flav = 0; flav < nFlav; ++flav){
                    uncHistMapUp[key][m][cat][flav] = std::vector < std::vector< std::shared_ptr< TH1D > > >(nDist);
                    uncHistMapDown[key][m][cat][flav] = std::vector < std::vector< std::shared_ptr< TH1D > > >(nDist);
                    for(unsigned dist = 0; dist < nDist; ++dist){
                        uncHistMapDown[key][m][cat][flav][dist] = std::vector< std::shared_ptr< TH1D > >(samples.size() + 1);
                        uncHistMapUp[key][m][cat][flav][dist] = std::vector< std::shared_ptr< TH1D > >(samples.size() + 1);
                        for(size_t sam = 0; sam < samples.size() + 1; ++sam){
                            ...
                            uncHistMapDown[key][m][cat][flav][dist][sam] = histInfo[dist].makeHist(flavNames[flav] + catNames[cat] + mllNames[m] + sampleName + key + "Down");
                            uncHistMapUp[key][m][cat][flav][dist][sam] = histInfo[dist].makeHist(flavNames[flav] + catNames[cat] + mllNames[m] + sampleName + key + "Up");
                        }
                    }
                }
            }
    ...
    std::vector< std::vector< std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > > > > pdfUncHists(100);
    for(unsigned pdf = 0; pdf < 100; ++pdf){
        pdfUncHists[pdf] = std::vector< std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > > >(nMll);
        for(unsigned m = 0; m < nMll; ++m){
            pdfUncHists[pdf][m] = std::vector< std::vector < std::vector< std::vector< std::shared_ptr< TH1D > > > > >(nCat); 
            for(unsigned cat = 0; cat < nCat; ++cat){
                pdfUncHists[pdf][m][cat] = std::vector< std::vector < std::vector< std::shared_ptr< TH1D > > > >( nFlav );
                for(unsigned flav = 0; flav < nFlav; ++flav){
                    pdfUncHists[pdf][m][cat][flav] =  std::vector < std::vector< std::shared_ptr< TH1D > > >( nDist );
                    for(unsigned dist = 0; dist < nDist; ++dist){
                        pdfUncHists[pdf][m][cat][flav][dist] = std::vector< std::shared_ptr< TH1D > >( samples.size() + 1);
                        for(size_t sam = 0; sam < samples.size() + 1; ++sam){
                            std::string sampleName;
                            if(sam < samples.size() ){
                                sampleName = samples[sam].getUniqueName();
                            ...
                            pdfUncHists[pdf][m][cat][flav][dist][sam] = histInfo[dist].makeHist(flavNames[flav] + catNames[cat] + mllNames[m] + sampleName + "pdf" + std::to_string(pdf) );
                        }
                    }
                }
            }
    ...
    for(size_t sam = 0; sam < samples.size(); ++sam){
        ...
        for(long unsigned it = 0; it < nEntries; ++it){
            ...
            //determine flavor category
            unsigned flavCat = trilep::flavorCompositionFourlepton(ind, _lFlavor, lCount);
            ...
            if(tzqCat > 2 && tzqCat < 6){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    hists[mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
                ...
                if( isData() && !passTightCut){
                    for(unsigned dist = 0; dist < nDist; ++dist){
                        for(auto& key: uncNames){
                            uncHistMapDown[key][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill( std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                            uncHistMapUp[key][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill( std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                        }
                        for(unsigned pdf = 0; pdf < 100; ++pdf){
                             pdfUncHists[pdf][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill( std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                        }
                    }
                }
            }
            ...
            if(tzqCatJECDown > 2 && tzqCatJECDown < 6){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapDown["JEC_2016"][mllCat][tzqCatJECDown - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            ...
            if(tzqCatJECUp > 2 && tzqCatJECUp < 6){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapUp["JEC_2016"][mllCat][tzqCatJECUp - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            }
            ...
            if(tzqCatUnclDown > 2 && tzqCatUnclDown < 6){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapDown["uncl"][mllCat][tzqCatUnclDown - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            }
            ...
            if(tzqCatUnclUp  > 2 && tzqCatUnclUp < 6){
                ...
                for(unsigned dist = 0; dist < nDist; ++dist){
                    uncHistMapUp["uncl"][mllCat][tzqCatUnclUp - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight);
                }
            }
            ...
            //vary scale down
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["scale"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*_lheWeight[8]);
            }

            //vary scale up
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["scale"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*_lheWeight[4]);
            }
            ...
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["bTag_udsg_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*bTagWeight_udsg(1)/bTagWeight_udsg(0));
            }

            //vary b-tag up for udsg
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["bTag_udsg_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*bTagWeight_udsg(2)/bTagWeight_udsg(0));
            }

            //vary b-tag down for b and c (correlated)
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["bTag_bc_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*bTagWeight_c(1)*bTagWeight_b(1)/ (bTagWeight_c(0)*bTagWeight_b(0)) );
            }

            //vary b-tag up for b and c (correlated)
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["bTag_bc_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*bTagWeight_c(2)*bTagWeight_b(2)/ (bTagWeight_c(0)*bTagWeight_b(0)) );
            }
            ...
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["lepton_reco"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*leptonRecoDownWeight );
            }

            //vary lepton reco SF up
            double leptonRecoUpWeight = leptonWeight("recoUp")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["lepton_reco"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*leptonRecoUpWeight);
            }

            //vary muon stat down
            double muonStatDownWeight = leptonWeight("muon_idStatDown")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["muon_id_stat_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*muonStatDownWeight);
            }

            //vary muon stat up
            double muonStatUpWeight = leptonWeight("muon_idStatUp")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["muon_id_stat_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*muonStatUpWeight );
            }

            //vary electron stat down
            double electronStatDownWeight = leptonWeight("electron_idStatDown")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["electron_id_stat_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*electronStatDownWeight );
            }

            //vary electron stat up
            double electronStatUpWeight = leptonWeight("electron_idStatUp")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["electron_id_stat_2016"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*electronStatUpWeight );
            }

            //vary lepton syst down
            double leptonIdSystDownWeight = leptonWeight("idSystDown")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapDown["lepton_id_syst"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*leptonIdSystDownWeight );
            }

            //vary lepton syst up
            double leptonIdSystUpWeight = leptonWeight("idSystUp")/leptonWeight("");
            for(unsigned dist = 0; dist < nDist; ++dist){
                uncHistMapUp["lepton_id_syst"][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*leptonIdSystUpWeight);
            }
            ...
            for(unsigned pdf = 0; pdf < 100; ++pdf){
                for(unsigned dist = 0; dist < nDist; ++dist){
                    pdfUncHists[pdf][mllCat][tzqCat - 3][flavCat][dist][fillIndex]->Fill(std::min(fill[dist], histInfo[dist].maxBinCenter() ), weight*_lheWeight[pdf + 9]);
                }
            }
        }
        ...
        for(unsigned m = 0; m < nMll; ++m){
            for(unsigned cat = 0; cat < nCat; ++cat){
                for(unsigned flav = 0; flav < nFlav; ++flav){
                    for(unsigned dist = 0; dist < nDist; ++dist){
                        analysisTools::setNegativeBinsToZero( hists[m][cat][flav][dist][sam].get() );
                        for(auto & key : uncNames){
                            analysisTools::setNegativeBinsToZero(uncHistMapDown[key][m][cat][flav][dist][sam].get() );
                            analysisTools::setNegativeBinsToZero(uncHistMapUp[key][m][cat][flav][dist][sam].get() );
                        } 
                        for(unsigned pdf = 0; pdf < 100; ++pdf){    
                            analysisTools::setNegativeBinsToZero(pdfUncHists[pdf][m][cat][flav][dist][sam].get() );
                        }
                    }	
                }
            }
        ...
        for( unsigned cat = 0; cat < nCat; ++cat){
            for(unsigned flav = 0; flav < nFlav; ++flav){
                for(unsigned dist = 0; dist < nDist; ++dist){
                    ...
                    analysisTools::setNegativeBinsToZero( hists[m][cat][flav][dist][nonpromptIndex].get() );
                    for(auto & key : uncNames){
                        analysisTools::setNegativeBinsToZero(uncHistMapDown[key][m][cat][flav][dist][nonpromptIndex].get() );
                        analysisTools::setNegativeBinsToZero(uncHistMapUp[key][m][cat][flav][dist][nonpromptIndex].get() );
                    } 
                    for(unsigned pdf = 0; pdf < 100; ++pdf){    
                        analysisTools::setNegativeBinsToZero(pdfUncHists[pdf][m][cat][flav][dist][nonpromptIndex].get() );
                    }
                }
            }
        }
            ...
            for(unsigned flav = 0; flav < nFlav; ++flav){
                for(unsigned dist = 0; dist < nDist; ++dist){
                    for(unsigned sam = 0; sam < samples.size() + 1; ++sam){
                        ...
                        for(unsigned bin = 1; bin < (unsigned) uncHistMapDown["pdf"][m][cat][flav][dist][sam]->GetNbinsX() + 1; ++bin){
                            double pdfVarRms = 0.;
                            for(unsigned pdf = 0; pdf < 100; ++pdf){

                                double variedBin = pdfUncHists[pdf][m][cat][flav][dist][sam]->GetBinContent(bin);
                                if( sam != samples.size() ){
                                    variedBin *= crossSectionRatio[sam][pdf + 9];
                                }
                                double diff = (  variedBin - hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                pdfVarRms += diff * diff;
                            }
                            pdfVarRms = sqrt( 0.01 * pdfVarRms );
                            uncHistMapDown["pdf"][m][cat][flav][dist][sam]->SetBinContent( bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) - pdfVarRms);
                            uncHistMapUp["pdf"][m][cat][flav][dist][sam]->SetBinContent( bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) + pdfVarRms);
                        }
                    }
                }
            }
        ...
        if( samples[sam].getFileName().find("GluGluToContinToZZ") == std::string::npos){
            for(unsigned m = 0; m < nMll; ++m){
                for(unsigned cat = 0; cat < nCat; ++cat){
                    for(unsigned flav = 0; flav < nFlav; ++flav){
                        for(unsigned dist = 0; dist < nDist; ++dist){
                            uncHistMapDown["scale"][m][cat][flav][dist][sam]->Scale(crossSectionRatio[sam][8]);
                            uncHistMapUp["scale"][m][cat][flav][dist][sam]->Scale(crossSectionRatio[sam][4]);
                        }
                    }
                }
            }
        ...
        for(unsigned cat = 0; cat < nCat; ++cat){
            for(unsigned flav = 0; flav < nFlav; ++flav){
                for(unsigned dist = 0; dist < nDist; ++dist){
                    for(unsigned sam = 0; sam < samples.size() + 1; ++sam){
                        if(sam < samples.size() ){
                            for(unsigned bin = 1; bin < (unsigned) uncHistMapDown["scaleXsec"][m][cat][flav][dist][sam]->GetNbinsX() + 1; ++bin){
                                uncHistMapDown["scaleXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, (1 + scaleXsecUncDown[sam])*hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapUp["scaleXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, (1 + scaleXsecUncUp[sam])*hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapDown["pdfXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, (1 + pdfXsecUncDown[sam])*hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapUp["pdfXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, (1 + pdfXsecUncUp[sam])*hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                            }
                        //WARNING: CURRENTLY THE SCALE AND PDF XSEC UNCERTAINTIES ON DATA-DRIVEN FAKES ARE SET TO ZERO
                        } else {
                            for(unsigned bin = 1; bin < (unsigned) uncHistMapDown["scaleXsec"][m][cat][flav][dist][sam]->GetNbinsX() + 1; ++bin){
                                uncHistMapDown["scaleXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapUp["scaleXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapDown["pdfXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                                uncHistMapUp["pdfXsec"][m][cat][flav][dist][sam]->SetBinContent(bin, hists[m][cat][flav][dist][sam]->GetBinContent(bin) );
                            }
                        }
                    }
                }
    ...
    std::vector< std::vector< std::vector< std::vector< std::vector< TH1D* > > > > > mergedHists(nMll);
    for(unsigned mll = 0; mll < nMll; ++mll){
        mergedHists[mll] = std::vector< std::vector< std::vector < std::vector < TH1D* > > > >(nCat);
        for(unsigned cat = 0; cat < nCat; ++cat){
            mergedHists[mll][cat] = std::vector< std::vector < std::vector < TH1D* > > >(nFlav);
            for(unsigned flav = 0; flav < nFlav; ++flav){ 
                for(unsigned dist = 0; dist < nDist; ++dist){
                    mergedHists[mll][cat][flav].push_back(std::vector<TH1D*>(proc.size() ) );

                    //cut off loop before nonprompt contribution
                    for(size_t m = 0, sam = 0; m < proc.size() - 1; ++m){
                        mergedHists[mll][cat][flav][dist][m] = (TH1D*) hists[mll][cat][flav][dist][sam]->Clone();
                        while(sam < samples.size() - 1 && samples[sam].getProcessName() == samples[sam + 1].getProcessName() ){
                            mergedHists[mll][cat][flav][dist][m]->Add(hists[mll][cat][flav][dist][sam + 1].get());
                            ++sam;
                        }
                        ++sam;
                    }
                    ...
                    mergedHists[mll][cat][flav][dist][proc.size() - 1] = (TH1D*) hists[mll][cat][flav][dist][samples.size()].get()->Clone();
                }
            }
        }
    }
    ...
    std::map< std::string, std::vector< std::vector< std::vector< std::vector< std::vector< TH1D* > > > > > > mergedUncMapDown;
    std::map< std::string, std::vector< std::vector< std::vector< std::vector< std::vector< TH1D* > > > > > > mergedUncMapUp;
    for( auto& key : uncNames ){
        mergedUncMapDown[key] = std::vector< std::vector< std::vector< std::vector< std::vector< TH1D* > > > > >(nMll);
        mergedUncMapUp[key] = std::vector< std::vector< std::vector< std::vector< std::vector< TH1D* > > > > >(nMll);
        for(unsigned mll = 0; mll < nMll; ++mll){
            mergedUncMapDown[key][mll] = std::vector< std::vector< std::vector < std::vector < TH1D* > > > >(nCat);
            mergedUncMapUp[key][mll] = std::vector< std::vector< std::vector < std::vector < TH1D* > > > >(nCat);
            for(unsigned cat = 0; cat < nCat; ++cat){
                mergedUncMapDown[key][mll][cat] = std::vector< std::vector < std::vector < TH1D* > > >(nFlav);
                mergedUncMapUp[key][mll][cat] = std::vector< std::vector < std::vector < TH1D* > > >(nFlav);
                for(unsigned flav = 0; flav < nFlav; ++flav){
                    for(unsigned dist = 0; dist < nDist; ++dist){
                        mergedUncMapDown[key][mll][cat][flav].push_back(std::vector<TH1D*>(proc.size() ) );
                        mergedUncMapUp[key][mll][cat][flav].push_back(std::vector<TH1D*>(proc.size() ) );

                        //cut off loop before nonprompt contribution
                        for(size_t m = 0, sam = 0; m < proc.size() - 1; ++m){
                            mergedUncMapDown[key][mll][cat][flav][dist][m] = (TH1D*) uncHistMapDown[key][mll][cat][flav][dist][sam]->Clone();
                            mergedUncMapUp[key][mll][cat][flav][dist][m] = (TH1D*) uncHistMapUp[key][mll][cat][flav][dist][sam]->Clone();
                            while(sam < samples.size() - 1 && samples[sam].getProcessName() == samples[sam + 1].getProcessName() ){
                                mergedUncMapDown[key][mll][cat][flav][dist][m]->Add( uncHistMapDown[key][mll][cat][flav][dist][sam + 1].get());
                                mergedUncMapUp[key][mll][cat][flav][dist][m]->Add( uncHistMapUp[key][mll][cat][flav][dist][sam + 1].get());
                                ++sam;
                            }
                            ++sam;
                        }
                        ...
                        mergedUncMapDown[key][mll][cat][flav][dist][proc.size() - 1] = (TH1D*) uncHistMapDown[key][mll][cat][flav][dist][samples.size()]->Clone();
                        mergedUncMapUp[key][mll][cat][flav][dist][proc.size() - 1] = (TH1D*) uncHistMapUp[key][mll][cat][flav][dist][samples.size()]->Clone();
                    }
                }
            }
        }
    ...
    std::vector< std::vector< std::vector< std::vector< TH1D* > > > > totalSystUnc(nMll); //copy pointers to fix dimensionality of vector

    for( unsigned mll = 0; mll < nMll; ++mll){
        totalSystUnc[mll] = std::vector< std::vector< std::vector< TH1D* > > >(nCat);
        for(unsigned cat = 0; cat < nCat; ++cat){
            totalSystUnc[mll][cat] = std::vector< std::vector< TH1D* > >(nFlav);
			for(unsigned flav = 0; flav < nFlav; ++flav){
                totalSystUnc[mll][cat][flav] = std::vector< TH1D* >(nDist);
            	for(unsigned dist = 0; dist < nDist; ++dist){
            	    totalSystUnc[mll][cat][flav][dist] = (TH1D*) mergedHists[mll][cat][flav][dist][0]->Clone();

            	    for(unsigned bin = 1; bin < (unsigned) totalSystUnc[mll][cat][flav][dist]->GetNbinsX() + 1; ++bin){
            	        double binUnc = 0;
            	        ...
            	        for(auto& key: uncNames ){
            	            ...
            	            for(unsigned p = 1; p < proc.size(); ++p){
            	                ...
            	                double nominalContent = mergedHists[mll][cat][flav][dist][p]->GetBinContent(bin);
            	                double downVariedContent = mergedUncMapDown[key][mll][cat][flav][dist][p]->GetBinContent(bin);
            	                double upVariedContent = mergedUncMapDown[key][mll][cat][flav][dist][p]->GetBinContent(bin);
                    	...
                    	for( double unc : flatUnc ){
                    	    ...
                    	    for(unsigned p = 1; p < proc.size(); ++p){
                    	        ...
                    	        double binContent = mergedHists[mll][cat][flav][dist][p]->GetBinContent(bin);
                    	...
                    	for(auto& uncPair : backgroundSpecificUnc){
                    	    for(unsigned p = 1; p < proc.size(); ++p){
                    	        if(proc[p] == uncPair.first){
                    	            double var = mergedHists[mll][cat][flav][dist][p]->GetBinContent(bin)*(uncPair.second - 1.);
                    	            binUnc += var*var;
                    	        }
                    	    }
                    	}
                    	...
                    	totalSystUnc[mll][cat][flav][dist]->SetBinContent(bin, sqrt(binUnc) );
					}
                }
            }
        }
    ...
    for(unsigned cat = 0; cat < nCat; ++cat){
        for(unsigned p = 1; p < proc.size(); ++p){
            ...
            for(unsigned flav = 0; flav < nFlav; ++flav){
                sumOfWeights += mergedHists[0][cat][flav][0][p]->GetSumOfWeights();
            }
            scaling[cat][p] = postFitScaler.postFitScaling( sumOfWeights );
        }
    }
