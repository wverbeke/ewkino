...
void treeReader::setup(){
    ...
    histInfo = {
        ...
        HistInfo("deltaPhiWlepZ", "#Delta#Phi(lepton from W, Z)", 30, 0, 3.15),
        HistInfo("deltaPhiWLeptonTaggedBJet", "#Delta#Phi(lepton from W, tagged b-jet", 30, 0, 7),
...
void treeReader::Analyze(const Sample& samp, const long unsigned begin, const long unsigned end){
    ...
    if( is2016() ){
        treeOutputDirectory = "trainingTrees_tZq2016";
    } else {
    ...
    for(long unsigned it = begin; it < end; ++it){
        ...
        if(jetCount != 0){
            ...
            if( highestCSVv2I != 99 ) highestCSVv2Jet = jetV[highestCSVv2I]; 
        }
        if(bJetCount != 0){
