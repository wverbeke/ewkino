...
void treeReader::Analyze(){
    ...
    histInfo = {
        ...
        */
        ...
        std::make_tuple("pTMinJetMET", "min(P_{T}(jet + MET)) (GeV)", 30, 0, 300), 
        std::make_tuple("pTMaxJetMET", "max(P_{T}(jet + MET)) (GeV)", 30, 0, 400),
        std::make_tuple("pTMinBJetMET", "min(P_{T}(bjet + MET)) (GeV)", 30, 0, 300),
        std::make_tuple("pTMaxBJetMET", "max(P_{T}(bjet + MET)) (GeV)", 30, 0, 400),
    ...
    */
    ...
    Float_t minMlb, maxmTbJetMet, maxpTLeptonMet, pT3l, ht, m3l, mZ;
    Float_t eventWeight;
    ...
    for(unsigned m = 0; m < nMll; ++m){
        for(unsigned cat = 0; cat < nCat; ++cat){
            ...
            for(unsigned t = 0; t < 2; ++t){
                ...
                tree[t][cat][m]->Branch("maxmTbJetMet", &maxmTbJetMet, "maxmTbJetMet/F");
                tree[t][cat][m]->Branch("maxpTLeptonMet", &maxpTLeptonMet, "maxpTLeptonMet/F");
