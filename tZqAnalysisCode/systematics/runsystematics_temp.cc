...
std::string systematicType(const std::string systematic){
    ...
    if(systematic=="electronScale" or systematic=="electronRes"){
	return std::string("electronscale");
    }
...
void fillSystematicsHistograms(const std::string& pathToFile, const double norm, 
	...
	for(std::string systematic : systematics){
	    ...
	    // if the systematic is electronScale, special selections are needed as well
	    else if(systematic=="electronScale"){
		// do event selection and flattening with up variation
		Event eScaleUpEvent = event.electronScaleUpEvent();
                bool passup = true;
                if(!passES(eScaleUpEvent, eventselection, selection_type, "nominal")) passup = false;
                if(doCat){
                    int eventcategory = eventCategory(eScaleUpEvent, "nominal");
                    if( std::find(signalcategories.begin(),signalcategories.end(),
                            eventcategory)==signalcategories.end()){ passup = false; }
                }
                if(passup){
                    accvarmap = eventFlattening::eventToEntry(eScaleUpEvent, norm, reweighter,
                                    selection_type, frmap_muon, frmap_electron, "nominal",
                                    readBDT, reader);
                    double weight = accvarmap["_normweight"];
                    // fill the variables
                    for(std::string variable : variables){
                        fillVarValue(histMap[thisPName][variable]["electronScaleUp"],
                                     getVarValue(variable,accvarmap),weight);
                    }
		}
		// do event selection and flattening with down variation
                Event eScaleDownEvent = event.electronScaleDownEvent();
                bool passdown = true;
                if(!passES(eScaleDownEvent, eventselection, selection_type, "nominal")) passdown = false;
                if(doCat){
                    int eventcategory = eventCategory(eScaleDownEvent, "nominal");
                    if( std::find(signalcategories.begin(),signalcategories.end(),
                            eventcategory)==signalcategories.end()){ passdown = false; }
                }
                if(passdown){
                    accvarmap = eventFlattening::eventToEntry(eScaleDownEvent, norm, reweighter,
                                    selection_type, frmap_muon, frmap_electron, "nominal",
                                    readBDT, reader);
                    double weight = accvarmap["_normweight"];
                    // fill the variables
                    for(std::string variable : variables){
                        fillVarValue(histMap[thisPName][variable]["electronScaleDown"],
                                     getVarValue(variable,accvarmap),weight);
                    }   
                }
		// printouts for testing
                /*std::cout << "nominal collection:" << std::endl;
                for( const auto elPtr: event.muonCollection() ){
                    std::cout << *elPtr << std::endl;
                }
                std::cout << "scale up collection:" << std::endl;
                for( const auto elPtr: eScaleUpEvent.muonCollection() ){
                    std::cout << *elPtr << std::endl;
                }
                std::cout << "scale down collection:" << std::endl;
                for( const auto elPtr: eScaleDownEvent.muonCollection() ){
                    std::cout << *elPtr << std::endl;
                }*/
	
	    }
	    // IF systematic is electronRes, special selections are needed as well
	    else if(systematic=="electronRes"){
                // do event selection and flattening with up variation
                Event eResUpEvent = event.electronResUpEvent();
                bool passup = true;
                if(!passES(eResUpEvent, eventselection, selection_type, "nominal")) passup = false;
                if(doCat){
                    int eventcategory = eventCategory(eResUpEvent, "nominal");
                    if( std::find(signalcategories.begin(),signalcategories.end(),
                            eventcategory)==signalcategories.end()){ passup = false; }
                }
                if(passup){
                    accvarmap = eventFlattening::eventToEntry(eResUpEvent, norm, reweighter,
                                    selection_type, frmap_muon, frmap_electron, "nominal",
                                    readBDT, reader);
                    double weight = accvarmap["_normweight"];
                    // fill the variables
                    for(std::string variable : variables){
                        fillVarValue(histMap[thisPName][variable]["electronResUp"],
                                     getVarValue(variable,accvarmap),weight);
                    }
                }
                // do event selection and flattening with down variation
                Event eResDownEvent = event.electronResDownEvent();
                bool passdown = true;
                if(!passES(eResDownEvent, eventselection, selection_type, "nominal")) passdown = false;
                if(doCat){
                    int eventcategory = eventCategory(eResDownEvent, "nominal");
                    if( std::find(signalcategories.begin(),signalcategories.end(),
                            eventcategory)==signalcategories.end()){ passdown = false; }
                }
                if(passdown){
                    accvarmap = eventFlattening::eventToEntry(eResDownEvent, norm, reweighter,
                                    selection_type, frmap_muon, frmap_electron, "nominal",
                                    readBDT, reader);
                    double weight = accvarmap["_normweight"];
                    // fill the variables
                    for(std::string variable : variables){
                        fillVarValue(histMap[thisPName][variable]["electronResDown"],
                                     getVarValue(variable,accvarmap),weight);
                    }
                }
            }
...
int main( int argc, char* argv[] ){
    ...
    if( argc < 17 ){
        std::cerr << "### ERROR ###: runsystematics.cc requires at least 16 arguments to run...: ";
