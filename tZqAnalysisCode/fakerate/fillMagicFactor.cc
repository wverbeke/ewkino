...
void determineMagicFactor( const std::string& leptonFlavor, const std::string& year, 
    ...
    // set sample list and make tree reader
    std::string sampleList = "/user/llambrec/ewkino/AnalysisCode/samplelists/samples_magicfactor.txt";
...
int main( int argc, char* argv[] ){
	...
	int sampleIndex = std::stoi( argvStr[5] );
        determineMagicFactor( leptonFlavor, year, leptonMVA, mvaThreshold, sampleIndex );
    } else {
        std::cerr << argc - 1 << " command line arguments given, while 5 are expected." << std::endl;
        std::cerr << "< flavor > < year > < lepton MVA > < mva threshold > < sample index >" << std::endl;
    }   
    return 0;
}
