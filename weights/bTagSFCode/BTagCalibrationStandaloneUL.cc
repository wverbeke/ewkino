/*
UL version of BTagCalibration standalone
*/

// the difference with respect to pre-UL is a different naming convention in the input CSV files.
// see here: https://twiki.cern.ch/twiki/bin/view/CMS/BTagCalibration
// note: updates are done manually (not provided centrally).
// note: should check if switching to new json format is feasible.

#include "BTagCalibrationStandaloneUL.h"
#include <iostream>
#include <exception>
#include <algorithm>
#include <sstream>
...
std::cerr << "ERROR in BTagCalibration: "
          << "Invalid csv line; operation point: "
          << csvLine;
throw std::exception();
  }
  unsigned uop = 3;
  ...
  if ( jf!=5 && jf!=4 && jf!=0 ) {
std::cerr << "ERROR in BTagCalibration: "
          << "Invalid csv line; jet flavour: "
          << csvLine;
throw std::exception();
  }
  params = BTagEntry::Parameters(
    BTagEntry::OperatingPoint(uop),
    vec[1],
    vec[2],
