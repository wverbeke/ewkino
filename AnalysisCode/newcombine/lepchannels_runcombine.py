###################################################################################
# run the combine executable for inclusive measurement per lepton flavour channel #
###################################################################################
# note: this script belongs to the 'explicit splitting' method
#       (see also lepchannels_makedatacards.py)
#       implicit splitting method: not yet supported.
...
def get_po_map( channels, lepchannels ):
    ...
    # - lepchannels: a dict mapping channel suffixes (e.g. _ch0) to names (e.g. eee)
...
if __name__=="__main__":
	...
	card = cards[0]
        print('running combine for '+card)
	commands = []
	# write text2workspace commands with PO mapping option
	(pois,pomap) = get_po_map( cdict.values(), get_lepchannel_parameters()[1] )
