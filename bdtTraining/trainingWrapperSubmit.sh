...
for jetCat in 0bJets_01Jets 0bJets_2Jets 1bJet_01jets 1bJet_23Jets 1bJet_4Jets 2bJets; do
    for mllCat in onZ offZ; do
        ...
        echo "  mv dataset/weights/TMVAClassification_${method}.weights.xml bdtWeights/${jetCat}_${mllCat}_${method}.weights.xml" >> mvaTraining.sh
        echo "done">> mvaTraining.sh
        qsub mvaTraining.sh -lnodes=1:ppn=8 -l walltime=20:00:00
    done
done
rm mvaTraining.sh
