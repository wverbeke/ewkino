...
def fillhistogram(hist, value, weight=1, clipouterflow=True):
    ### add one value to a histogram, taking care of under- and overflow
    nbins = hist.GetNbinsX()
    xlow = hist.GetBinLowEdge(1)
    xhigh = hist.GetBinLowEdge(nbins)+hist.GetBinWidth(nbins)
    if(clipouterflow and value>xhigh):
	value = xhigh - hist.GetBinWidth(nbins)/2.
    if(clipouterflow and value<xlow):
	value = xlow + hist.GetBinWidth(0)/2.
