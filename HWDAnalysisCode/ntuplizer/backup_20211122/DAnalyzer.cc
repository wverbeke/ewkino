...
void DAnalyzer::beginJob(TTree* outputTree){
    ...
    outputTree->Branch("_DTwoTrackSeparationX", &_DTwoTrackSeparationX, "_DTwoTrackSeparationX[_nDs]/D");
    outputTree->Branch("_DTwoTrackSeparationY", &_DTwoTrackSeparationY, "_DTwoTrackSeparationY[_nDs]/D");
    outputTree->Branch("_DTwoTrackSeparationZ", &_DTwoTrackSeparationZ, "_DTwoTrackSeparationZ[_nDs]/D");
    outputTree->Branch("_DTrackVtxSeparationX", &_DTrackVtxSeparationX, "_DTrackVtxSeparationX[_nDs]/D");
    outputTree->Branch("_DTrackVtxSeparationY", &_DTrackVtxSeparationY, "_DTrackVtxSeparationY[_nDs]/D");
    outputTree->Branch("_DTrackVtxSeparationZ", &_DTrackVtxSeparationZ, "_DTrackVtxSeparationZ[_nDs]/D");
    // track variables
...
void DAnalyzer::analyze(const edm::Event& iEvent, const reco::Vertex& primaryVertex){
    ...
    for( std::map<std::string,double> temp: dcandidates ){
	    ...
	    // track combination variables
	    _DTwoTrackSeparationX[_nDs] = temp["twotracksepx"];
	    _DTwoTrackSeparationY[_nDs] = temp["twotracksepy"];
	    _DTwoTrackSeparationZ[_nDs] = temp["twotracksepz"];
	    _DTrackVtxSeparationX[_nDs] = temp["trackvtxsepx"];
	    _DTrackVtxSeparationY[_nDs] = temp["trackvtxsepy"];
	    _DTrackVtxSeparationZ[_nDs] = temp["trackvtxsepz"];
	    // track variables
...
std::vector< std::map<std::string,double> > DAnalyzer::DsFitter(
	...
	const reco::Track tr2 = ordered ? tracks.at(tr2index) : tracks.at(tr1index);
        // apply additional quality cuts on the tracks
        if( tr1.pt()<2.0 || tr2.pt()<2.0 ) continue;
        if( !tr1.quality(reco::TrackBase::qualityByName("highPurity"))
           || !tr2.quality(reco::TrackBase::qualityByName("highPurity")) ) continue;
        // cuts on the two-track combination
        if( tr1.charge()*tr2.charge()>0 ) continue;
	...
	ROOT::Math::PxPyPzMVector resvector = tr1vec + tr2vec;
        ...
        for(unsigned tr3index=0; tr3index<tracks.size(); tr3index++){
            ...
            const reco::Track tr3 = tracks.at(tr3index);
            // additional quality cuts on the third track
            if( tr3.pt()<2.0 ) continue;
            // selections on the track-resonance combination
            const math::XYZPoint tr3refpoint = tr3.referencePoint();
	    double trackvtxsepx = std::abs(tr3refpoint.x()-resvtx.position().x());
	    double trackvtxsepy = std::abs(tr3refpoint.y()-resvtx.position().y());
	    double trackvtxsepz = std::abs(tr3refpoint.z()-resvtx.position().z());
		...
		{"resvtxnormchi2",0.},
		// track combination variables
		{"twotracksepx", 0.}, {"twotracksepy", 0.}, {"twotracksepz", 0.},
		{"trackvtxsepx", 0.}, {"trackvtxsepy", 0.}, {"trackvtxsepz", 0.},
		// track variables
	    ...
	    // track combination variables
	    dcandidate["twotracksepx"] = twotracksepx;
	    dcandidate["twotracksepy"] = twotracksepy;
	    dcandidate["twotracksepz"] = twotracksepz;
	    dcandidate["trackvtxsepx"] = trackvtxsepx;
	    dcandidate["trackvtxsepy"] = trackvtxsepy;
	    dcandidate["trackvtxsepz"] = trackvtxsepz;
	    // track variables
